        ...
        Returns:
            tuple[str, Severity, dict[str, str | int | float | bool | None]]:
                - `drift_type`: Classification string describing the detected drift
                    (for example, `"lock_lost"` or an inconsistency type value).
                ...
                - `metadata`: A dictionary with contextual information. Always includes
                    ...
                    `detected_at` (when applicable). Normal evaluations also include
                    `job_id`, `reason`, and job-specific fields from `_build_job_metadata()`.
        """
