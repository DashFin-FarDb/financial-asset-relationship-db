...
EDGE_COORD_STRIDE: int = (
    3  # slots per edge in pre-allocated coordinate arrays (src, tgt, None)
)
...
def _get_line_style(rel_type: str, is_bidirectional: bool) -> dict:
    ...
    return dict(
        color=color,
        width=LINE_WIDTH_BIDIRECTIONAL
        if is_bidirectional
        else LINE_WIDTH_UNIDIRECTIONAL,
...
def _create_directional_arrows(
    ...
    arrow_positions = source_positions + ARROW_POSITION_RATIO * (
