...
def create_sample_database() -> AssetRelationshipGraph:
    ...
    try:
        ...
        logger.info(
            "Expanded sample database created with %s assets and "
            "%s relationships",
            asset_count,
            relationship_count,
        )
        logger.info(
            ...
            len(
                [a for a in all_assets if a.asset_class == AssetClass.EQUITY]
            ),
            len(
                [a for a in all_assets if a.asset_class == AssetClass.FIXED_INCOME]
            ),
            len(
                [a for a in all_assets if a.asset_class == AssetClass.COMMODITY]
            ),
            len(
                [a for a in all_assets if a.asset_class == AssetClass.CURRENCY]
            ),
        )
