...
class FinancialAssetApp:
        ...
        for name in candidates:
            ...
            if callable(fn):
                ...
                if isinstance(graph, AssetRelationshipGraph):
                    return graph
                raise TypeError(f"{name}() returned {type(graph)!r}, " f"expected AssetRelationshipGraph")
    ...
    def _format_formula_summary(summary: dict[str, Any], analysis_results: dict[str, Any]) -> str:
        ...
        summary_lines: list[str] = [
            ...
            "",
            f"📊 **Total Formulas Identified:** " f"{len(formulas) if isinstance(formulas, list) else 0}",
            f"📈 **Average Reliability (R²):** " f"{float(summary.get('avg_r_squared', 0.0)):.3f}",
            f"🔗 **Empirical Data Points:** " f"{int(summary.get('empirical_data_points', 0))}",
            "",
            "📋 **Formula Categories:**",
        ]
