...
- **Backend required env vars:** importing `api.main` needs `DATABASE_URL`, `SECRET_KEY`, `ADMIN_USERNAME`, `ADMIN_PASSWORD` (see `run-dev.sh`). Example working start:
  `DATABASE_URL=sqlite:dev.db SECRET_KEY=$(openssl rand -hex 32) ADMIN_USERNAME=admin ADMIN_PASSWORD=$(openssl rand -base64 24) python -m uvicorn api.main:app --reload --host 127.0.0.1 --port 8000`
