...
- **SQLite `DATABASE_URL` gotcha (backend won't start otherwise):** this repository uses a **custom URL-to-path
  resolver** in `api/database.py` (`_resolve_file_path`), *not* SQLAlchemy's URL handling. Under that resolver the
  SQLAlchemy-style `sqlite:///./dev.db` used in the "Common commands" section resolves to the absolute path `/dev.db`
  ...
  (resolves relative to `$PWD`, i.e. `/workspace/dev.db` when run from the repository root -- this is what
...
- **Known pre-existing test failures (not environment issues):** `tests/unit/test_workflow_yaml_files.py` fails on
  `codeql.yml` (via `pytest.fail`) because `.github/workflows/codeql.yml` does not exist in the repository. The rest of
  the suite passes (~7777 passed).
