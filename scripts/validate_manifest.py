...
def _collect_headings(lines: list[str]) -> dict[str, list[int]]:
    ...
    Returns:
        Dict[str, List[int]]: Mapping from each level-2 heading text
            (the text following '##') to a list of 1-based line numbers
            where that heading appears.
    """
...
def _report_duplicates(
    ...
    Parameters:
        duplicates (Dict[str, List[int]]): Mapping from duplicated heading
            text to the list of 1-based line numbers where each occurs.
...
def check_duplicate_headings(manifest_path: Path) -> int:
    ...
    Verifies that manifest_path resolves to the repository's
    .elastic-copilot/memory/systemManifest.md, reads the file, and detects
    duplicate level-2 headings (lines starting with "## " but not "### ").
    When duplicates are found or validation fails, a formatted report is
    written to stderr.

    Parameters:
        manifest_path (Path): Path to the systemManifest.md file to validate;
            must resolve to the repository's expected manifest location.

    Returns:
        int: Exit code where `0` indicates no duplicate level-2 headings were
            found, and `1` indicates a missing or unexpected path, a
            read/validation failure, or that duplicates were detected.
    """
...
def main():
    """Run the duplicate-heading validation script."""
