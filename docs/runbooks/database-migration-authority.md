...
### PostgreSQL request statement timeout

- **Configuration:** Ordinary PostgreSQL API request connections apply a server-side `statement_timeout`, configured
  by `POSTGRES_REQUEST_STATEMENT_TIMEOUT_MILLISECONDS` and defaulting to `120000` (120 seconds).
- **Validation:** The value must be an integer from `1` through PostgreSQL's `2147483647` millisecond maximum. Zero,
  negative, over-limit, empty, and non-integer explicit values fail settings validation.
- **Operational trade-off:** Lower values release locks and connections sooner but may cancel legitimate long-running
  graph or administrative statements. Higher values allow more work but retain blocked resources longer.
- **Startup override:** The read-only startup verifier keeps its independent 15-second bound and overrides the request
  setting while its operation guard is active.
- **Error boundary:** A PostgreSQL query-cancellation exception propagates through the existing DB-API boundary and the
  connection is closed. This setting adds no new HTTP status or protected diagnostic contract.
- **Database-specific exceptions:** SQLite and explicit migration connections retain their existing behavior.
