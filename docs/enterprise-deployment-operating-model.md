...
- `REBUILD_LOCK_TTL_SECONDS`: Time-to-live for the graph rebuild distributed lock in seconds (default: `300`). Must be a positive integer. Loaded via `src/config/settings.py` and propagated to graph rebuild orchestration. The heartbeat refresh interval is `max(1, lock_ttl_seconds // 3)` per [ADR 0003](adr/0003-lock-refresh-and-heartbeat.md).
