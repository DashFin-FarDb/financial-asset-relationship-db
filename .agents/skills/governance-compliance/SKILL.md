...
4. Accept mandatory evidence only when it is an executed pass for the exact
   head and target. Skipped, canceled, unavailable, stale, and wrong-target
   results do not pass.
...
6. Carry unresolved findings forward. Classify validated outcomes as resolved,
   deferred out of scope, rejected speculative, duplicate, recurrence, waived,
   or stale. Thread resolution alone is not remediation.
