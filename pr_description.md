...
- Converted `.circleci/config.yml` to a stub as part of CI platform deduplication (CircleCI is no longer in use; GitHub Actions is the canonical PR gate). Any legacy branch protection rules that still reference CircleCI must be updated to reference GitHub Actions required checks instead.
