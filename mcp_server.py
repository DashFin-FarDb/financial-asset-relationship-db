...
def main(argv: list[str] | None = None) -> int:
    """
    Run the MCP server command-line interface.

    Parses optional CLI arguments (currently `--version`); if `--version` is provided prints version information and exits. Otherwise attempts to build and run the MCP application; if the optional MCP dependency is missing, raises SystemExit with a message indicating the missing package.

    Parameters:
        argv (list[str] | None): Optional list of command-line arguments to parse. If None, arguments are read from the environment.

    Returns:
        int: Exit status code (0 on success).

    Raises:
        SystemExit: If the MCP dependency is missing or when exiting after printing version information.
    """
