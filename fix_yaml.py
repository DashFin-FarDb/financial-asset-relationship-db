...
def fix_release_evidence():
    ...
    content = content.replace(
        "actions/upload-artifact@5d5d22a31266ced268874388b761e4b58bb5c2f3 # v4",
        "actions/upload-artifact@6543ce64cb6eb0d110ebf6fb5b172aeb41d3b5b6 # v4",
    )
...
"""
    content = content.replace(
        "      - name: Run Gate Tests (Security)", doc_step + "      - name: Run Gate Tests (Security)"
    )
...
"""
    content = content.replace(
        ...
        tag_parsing + "      - name: Verify Staging Baseline Checklist",
    )
...
"""
    content = content.replace(
        "      - name: Assert Persistence Loaded", audit + "      - name: Assert Persistence Loaded"
    )
