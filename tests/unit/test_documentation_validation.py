...
class TestDependencyMatrix:
    ...
    def test_dependency_matrix_has_generated_timestamp(self, dependency_matrix_content):
        ...
        This test looks for a line matching the pattern '*Generated: YYYY-MM-DDTHH:MM:SS.sssZ*' (for example
        '*Generated: 2025-11-07T18:22:38.791Z*') and asserts the captured timestamp can be parsed as a valid ISO 8601
        instant.
    ...
    def test_dependency_matrix_has_file_types(self, dependency_matrix_content):
        ...
        Asserts the presence of a "- File types: ..." entry in the provided document and that it lists at least one file
        type. Also ensures every listed type is within the allowed set(py, js, ts, tsx) or permitted extras(jsx, json,
        md).

        Parameters:
            dependency_matrix_content(str): Full text content of .elastic - copilot / memory / dependencyMatrix.md to
            inspect.
        """
    ...
    def test_dependency_matrix_file_counts_match(self, dependency_matrix_content):
        ...
        Asserts that the integer parsed from "- Files analyzed: N" matches the sum of all integers parsed from lines
        like "- X <type> files".
        """
    ...
    def test_dependency_matrix_dependency_format(self, dependency_matrix_content):
        ...
        For each "Top dependencies:" section in the provided document, ensures the section content(up to the next "###"
        heading) either contains the message "No common dependencies found" or consists of non - empty lines that start
        with a dash(`-`).
    ...
    def test_dependency_matrix_no_empty_dependency_sections(self, dependency_matrix_content):
        ...
        Parameters:
            ...
            the test splits this by the "Top dependencies:" marker and asserts that the text preceding the next "###"
            heading is not empty for each section.
        """
    ...
    def test_dependency_matrix_markdown_formatting(self, dependency_matrix_lines):
        ...
        Raises:
            AssertionError: If a heading line(one or more '#' characters followed by content) does not have a space
            after the hashes
...
class TestSystemManifest:
    ...
    def test_system_manifest_has_project_description(self, system_manifest_content):
        """Verify the system manifest contains a '- Description: ...' entry."""
    ...
    def test_system_manifest_file_counts(self, system_manifest_content):
        ...
        Checks for lines matching the pattern "- N <type> files" and asserts at least one such line exists. Fails if any
        extracted count is negative.
        """
        pattern = r"- (\d+) (\w+) files"
    ...
    def test_system_manifest_file_dependency_format(self, system_manifest_content):
        r"""
        Validate that file - level dependency headers in the system manifest follow the expected path - and -extension
        format.

        Searches the document for headers of the form "### \\path\\to\\file.ext" and asserts that each matched header
        contains a path separator(`\\` or `/`). Only the first 10 matches are checked for performance.
    ...
    def test_system_manifest_dependency_entries_have_content(self, system_manifest_content):
        ...
        Splits the manifest by file headers introduced with "###" and inspects up to the first 20 file sections. For
        each non - empty file section(excluding sections that start with "#"), asserts the section contains either the
        literal "Dependencies:", the literal "No dependencies found", or begins with a backslash(indicating a file
        path).
    ...
    def test_system_manifest_markdown_formatting(self, system_manifest_lines):
        ...
        Asserts that, within the first 500 lines, any Markdown heading that begins with one or more `  # ` characters
        has a space immediately following the leading hash sequence (e.g. `# Title`, `## Section`). The test raises an
        assertion identifying the line number and content when a heading is missing the required space.
        """
...
class TestDocumentationConsistency:
    ...
    def test_file_counts_match_between_documents(self, dependency_matrix_content, system_manifest_content):
        """
        Verify that per - type file counts are equal between dependencyMatrix.md and the System Manifest's Project
        Structure section.

        Parses patterns of the form "- N <type> files" from dependency_matrix_content and from the first "## Project
        Structure" section of system_manifest_content, then asserts that counts match for each file type present in both
        documents.
        ...
        Raises:
                AssertionError: If the "## Project Structure" section is missing or if any file - type counts differ
    ...
    def test_file_types_match_between_documents(self, dependency_matrix_content, system_manifest_content):
        """
        Verify that the set of file types listed in the dependency matrix matches the set reported in the system
        manifest.

        This test compares the "File types" entry from dependencyMatrix.md with the file type names extracted from the
        "Project Structure" section of systemManifest.md and fails if the two sets differ.
        """
    ...
    def test_timestamps_are_recent(self, dependency_matrix_content, system_manifest_content):
        ...
        Checks the dependency matrix "Generated" timestamp and the system manifest "Last Updated" timestamp(expected as
        ISO 8601 with milliseconds and a trailing "Z"); if either timestamp is present and is more than one year old the
        test fails.
        """
...
class TestDocumentationRealisticContent:
    ...
    def test_documented_files_exist(self):
        ...
        Searches the manifest for file entries formatted as "### \\path\\to\\file.ext" (common Python, TS / TSX, JSX /
        JSX patterns), normalises Windows - style backslashes to POSIX paths, strips any leading slash, and checks
        existence for up to the first 20 discovered paths. Entries that are placeholders or clearly test -
        related(containing "...", "test_", or "__tests__") are skipped.
        """
    ...
    def test_documented_dependencies_are_real_packages(self):
        """
        Validate that dependencies listed in .elastic - copilot / memory / dependencyMatrix.md resemble real package
        names.

        Reads the dependency matrix, extracts bullet - list entries, filters out lines referring to file counts or
        metadata, and asserts that the first 20 candidate dependencies match common package - name
        patterns(alphanumeric, dot, dash, underscore, scoped names, and simple path - like entries).
        """
        matrix_path = Path(".elastic-copilot/memory/dependencyMatrix.md")
        with open(matrix_path, encoding="utf-8") as f:
            content = f.read()
