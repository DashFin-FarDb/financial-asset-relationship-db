...
class TestCacheHelpers:
    ...
    def test_load_from_cache_success(self):
        ...
        with (
            patch("src.data.real_data_fetcher.Path.open") as mock_open,
            patch(
                "src.data.real_data_fetcher.json.load", return_value=cache_payload
            ) as mock_json_load,
            patch(
                "src.data.real_data_fetcher._deserialize_graph",
...
class TestRealDataFetcher:
    ...
    @patch("src.data.real_data_fetcher.yf.Ticker")
    def test_fetch_multiple_stocks_success(
        self, mock_ticker, fetcher, mock_ticker_data
    ):
    ...
    @patch("src.data.real_data_fetcher.yf.Ticker")
    def test_fetch_multiple_stocks_partial_failure(
        self, mock_ticker, fetcher, mock_ticker_data
    ):
    ...
    def test_fetch_with_invalid_cache(
        self, mock_save, mock_ticker, mock_is_valid, fetcher, mock_ticker_data
    ):
...
class TestIntegrationWithAssetGraph:
    ...
    def test_graph_population(self, mock_fetch):
        ...
        all_ids = set(graph.relationships.keys()) | {
            t for v in graph.relationships.values() for t, _, _ in v
        }
