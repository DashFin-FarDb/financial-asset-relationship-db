...
@pytest.fixture
def sample_user():
    """Create a sample UserInDB instance for tests."""
    return UserInDB(
        username="testuser",
...
class TestIsTruthy:
    ...
    def test_is_truthy_with_zero(self):
        """Test that '0' is not considered truthy."""
        assert _is_truthy("0") is False

    def test_is_truthy_with_empty_string(self):
        """Test that an empty string is not considered truthy."""
        assert _is_truthy("") is False

    def test_is_truthy_with_none(self):
        """Test that None is not considered truthy."""
...
class TestUserRepository:
    ...
    def test_get_user_not_found(self, mock_fetch_one):
        """Test retrieval of a user that does not exist."""
    ...
    def test_has_users_returns_true(self, mock_fetch_value):
        """Test if has_users returns True when users exist."""
    ...
    def test_create_or_update_user_with_all_fields(self, mock_execute):
        """Test creating or updating a user with all fields."""
        repo = UserRepository()
...
class TestAuthenticateUser:
    ...
    def test_authenticate_user_wrong_password(self, mock_user_repository):
        """Test authentication failure with a wrong password."""
        password = "correct_password"
        hashed_password = get_password_hash(password)
        user = UserInDB(
            username="testuser",
...
class TestGetCurrentActiveUser:
    ...
    async def test_get_current_active_user_with_disabled_user(self, sample_user):
        """Test getting the current active user when the user is disabled."""
