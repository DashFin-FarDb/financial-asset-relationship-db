...
def test_start_safety_covers_concurrency_identity_port_ownership_and_rollback() -> None:
    ...
    for expected in (
        ...
        "Test-ActiveUnitExecStartMatch",
        "-Property 'ExecStart'",
        "argv\\[\\]",
        "Test-ActiveTransientUnitReusable",
        "if ($unitState -eq 'inactive') { return $false }",
        "The transient application unit is in a pre-existing non-startable state",
        ...
        "$script:FrontendInstallManifestPath",
        "$pythonRuntimeFingerprint = Get-WslFileSha256 -Path $script:PythonPath",
        "Get-WslNpmImplementationRoot",
        "$npmImplementationFingerprint = Get-NativeStreamSha256",
        "'The installed npm implementation bytes could not be fingerprinted safely.'",
        "$nodeRuntimeFingerprint = Get-WslFileSha256 -Path $script:NodePath",
        "'ls', '--all', '--json'",
        "-AllowedExitCodes @(0, 1)",
        "'pip', 'list', '--format=json', '--disable-pip-version-check'",
        "$output.Length -gt (4 * 1024 * 1024)",
        "'/usr/bin/sha256sum'",
        "Invoke-LocalGit",
        ...
        "if ($InitialStates.Pdc -eq 'inactive')",
        "if ($InitialStates.Prometheus -eq 'inactive')",
        "if ($InitialState -ne 'inactive') { return }",
        'Write-Error "Rollback cleanup could not stop the newly started $Label unit: $Unit"',
        "Rollback cleanup could not stop exact infrastructure units",
        "Get-BackendStartSpec",
        "Get-FrontendStartSpec",
        "The active transient unit was started from different runtime inputs or installed dependency state",
