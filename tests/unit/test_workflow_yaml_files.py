...
# --- Tests for PR-specific changes ---


class TestCircleCINodeExecutorVersion:
    """Tests for the node-executor Docker image version change.

    This PR changed the node-executor image from cimg/node:25.9.0 to
    cimg/node:25.8.1.
    """
    ...
    def test_node_executor_image_is_25_8_1(self, circleci_config):
        """node-executor must use the pinned image cimg/node:25.8.1."""
...
class TestCircleCISafetyCheckCommand:
    """Tests for the safety check command change.

    This PR changed the security check from:
        safety check --file requirements.txt --json --ignore 72086
    to:
        safety check --json --ignore 72086

    The --file flag was removed so safety scans the whole environment instead.
    """
    ...
    @staticmethod
    def _get_safety_check_command(circleci_config: dict) -> str:
        """Extract the safety check command from the python-security job."""
        security_job = circleci_config["jobs"]["python-security"]
        for step in security_job["steps"]:
            if isinstance(step, dict) and "run" in step:
                ...
                if isinstance(run_block, dict):
                    ...
                    if "safety" in name.lower() or "safety check" in command:
                        return command
        return ""

    def test_safety_check_command_has_no_file_flag(self, circleci_config):
    ...
    def test_safety_check_command_has_json_flag(self, circleci_config):
        ...
        command = self._get_safety_check_command(circleci_config)
        assert command, "safety check step not found in python-security job"
        assert "--json" in command, "safety check should use --json output format"

    def test_safety_check_command_has_ignore_flag(self, circleci_config):
