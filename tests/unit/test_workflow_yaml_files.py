...
class TestSpecificWorkflows:
    ...
    def test_ci_security_scanner_lock_is_exact_and_hashed(self):
        ...
        for requirement_line in source_lines:
            assert requirement_line.count("==") == 1, f"scanner root must use one exact pin: {requirement_line}"
            package_name, version = requirement_line.split("==", maxsplit=1)
            assert package_name and version, f"scanner root must name a package and version: {requirement_line}"
            source_versions[package_name.lower().replace("_", "-")] = version
        ...
        for position, start in enumerate(requirement_starts):
            end = requirement_starts[position + 1] if position + 1 < len(requirement_starts) else len(lock_lines)
            requirement_line = lock_lines[start]
            ...
            assert exact_requirement.count("==") == 1, f"lock entry must use one exact pin: {requirement_line}"
            package_name, version = exact_requirement.split("==", maxsplit=1)
            assert package_name and version, f"lock entry must name a package and version: {requirement_line}"
            ...
            locked_versions[package_name.lower().replace("_", "-")] = version
