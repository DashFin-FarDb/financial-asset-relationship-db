...
class TestSettingsModel:
    ...
    def test_production_allows_missing_admin_password(self) -> None:
        """Test that production runtime starts without migration-only ADMIN_PASSWORD."""
        settings = Settings(
            env=DeploymentEnvironment.PRODUCTION,
            secret_key="secret-key-that-is-at-least-32-bytes",
            admin_username="admin",
...
class TestLoadSettings:
    ...
    def test_load_settings_allows_production_without_admin_password(self) -> None:
        """Test that production runtime settings load without ADMIN_PASSWORD."""
        settings = load_settings()
        assert settings.env == DeploymentEnvironment.PRODUCTION
        assert settings.admin_username == "admin"
        assert settings.admin_password is None
