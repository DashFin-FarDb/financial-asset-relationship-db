...
def production_promotion_workflow_fixture() -> dict:
    ...
    if not isinstance(data, dict):
        pytest.fail("production-promotion.yml must parse to a mapping")
...
def test_production_promotion_is_manual_dispatch_only(production_promotion_workflow: dict) -> None:
    ...
    ASSERTIONS.assertEqual(set(production_promotion_workflow["on"]), {"workflow_dispatch"})
...
def test_production_promotion_requires_main_before_environment(
    ...
    if isinstance(needs, list):
        ...
        ASSERTIONS.assertEqual(needs, "require-main")
...
def test_production_promotion_uses_job_level_permissions(production_promotion_workflow: dict) -> None:
    ...
    ASSERTIONS.assertEqual(production_promotion_workflow["jobs"]["promotion-gate"]["permissions"], {"contents": "read"})
...
def test_production_promotion_binds_readiness_to_environment_secret(
    ...
    ASSERTIONS.assertIn('BASE_URL_INPUT" != "$HOSTED_READINESS_BASE_URL"', production_promotion_raw)
...
def test_production_promotion_unsets_empty_untrusted_roles_env(
    ...
    ASSERTIONS.assertIn("unset FARDB_UNTRUSTED_DATABASE_ROLES", production_promotion_raw)
    ASSERTIONS.assertIn('if [ -z "$FARDB_UNTRUSTED_DATABASE_ROLES" ]', production_promotion_raw)
...
def test_production_promotion_uploads_production_artifacts(
    ...
    ASSERTIONS.assertEqual(len(upload_steps), 1)
    ASSERTIONS.assertEqual(upload_steps[0]["with"]["name"], "production-readiness")
