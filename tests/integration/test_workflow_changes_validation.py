...
class TestWorkflowActionVersionRefs:
    ...
    def _load_workflow(filename: str) -> dict:
        """Load a workflow YAML file using a loader that handles the 'on' key correctly."""
        import copy
        import re as _re
        ...
        class _GHLoader(_yaml.SafeLoader):
            pass
        ...
        with open(workflow_path, "r") as f:
            return _yaml.load(f, Loader=_GHLoader)

    @staticmethod
    def _collect_uses_refs(workflow: dict) -> list:
        """Gather all 'uses' values from all job steps in a workflow."""
        refs = []
    ...
    def test_auto_assign_workflow_uses_semantic_ref(self):
        """auto-assign.yml should use a shortened SHA or semantic version ref."""
        workflow = self._load_workflow("auto-assign.yml")
        refs = self._collect_uses_refs(workflow)
        assign_refs = [r for r in refs if "pozil/auto-assign-issue" in r]
        assert assign_refs, "auto-assign.yml must reference pozil/auto-assign-issue action"
        # The PR changed this to a shortened SHA; must still have @ version separator
        ref = assign_refs[0]
    ...
    def test_autofix_workflow_uses_semantic_checkout(self):
        ...
        workflow = self._load_workflow("autofix.yml")
        refs = self._collect_uses_refs(workflow)
    ...
    def test_ci_workflow_checkout_uses_semantic_version(self):
        ...
        refs = self._collect_uses_refs(workflow)
        checkout_refs = [r for r in refs if "actions/checkout" in r]
        assert checkout_refs, "ci.yml must contain actions/checkout step(s)"
    ...
    def test_debricked_workflow_checkout_uses_pinned_sha(self):
        ...
        The debricked workflow explicitly kept a SHA pin for supply-chain security.
        """
        workflow = self._load_workflow("debricked.yml")
        refs = self._collect_uses_refs(workflow)
        checkout_refs = [r for r in refs if "actions/checkout" in r]
        assert checkout_refs, "debricked.yml must contain an actions/checkout step"
        ref = checkout_refs[0]
    ...
    def test_dependency_review_workflow_uses_semantic_action_ref(self):
        ...
        workflow = self._load_workflow("dependency-review.yml")
        refs = self._collect_uses_refs(workflow)
