...
class TestPrAgentWorkflow:
    ...
    def test_pr_agent_name(self, pr_agent_workflow: dict[str, Any]):
        """
        Assert the pr-agent workflow name mentions pr agent.

        The top-level "name" must contain "pr agent"
    ...
    def test_pr_agent_checkout_has_token(self, pr_agent_workflow: dict[str, Any]):
        """
        Ensure every checkout step provides a token.

        Every actions/checkout step in the pr-agent-action job must provide a
    ...
    def test_pr_agent_has_python_setup(self, pr_agent_workflow: dict[str, Any]):
        """
        Assert the pr-agent-action job sets up Python.

        The job must include at least one step that uses
    ...
    def test_pr_agent_python_version(self, pr_agent_workflow: dict[str, Any]):
        """
        Ensure setup-python uses Python 3.11.

        Any actions/setup-python step in the "pr-agent-action" job must specify
        python-version "3.11".
