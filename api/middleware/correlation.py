...
def _extract_and_validate_id(raw_id: str | None, header_name: str) -> str | None:
    """
    Validate and return a trimmed identifier extracted from an incoming header,

    or `None` if the value is not acceptable.

    If `raw_id` is not a string, the function returns `None`.

    Values whose original length exceeds `MAX_HEADER_LENGTH` are rejected

    and a `correlation_id_oversized_header` observability event is emitted.

    The header value is trimmed of surrounding whitespace and validated;

    if validation fails, a `correlation_id_invalid_header` observability event is emitted.
...
def _inject_state(scope: Scope, request_id: str, correlation_id: str) -> None:
    ...
    - If the state object is a mapping, it first tries mapping-style writes
    and falls back to attribute-style assignment on failure.
    - If the state object is not a mapping, it attempts attribute-style assignment only.
    - Failures to attach IDs do not raise; the function logs structured observability events for skipped injection,
    mapping errors, attribute-assignment failures, and other unexpected errors.
