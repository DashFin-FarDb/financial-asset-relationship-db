...
def _extract_and_validate_w3c_id(raw_id: str | None, header_name: str, regex: Pattern[str]) -> str | None:
    ...
    Note: W3C requires trace and span IDs to be lowercase hex strings and explicitly
    rejects all-zero values.
    """
    trimmed_id = _extract_and_validate_id(raw_id, header_name)
    if trimmed_id is None:
        return None

    if not regex.match(trimmed_id) or trimmed_id == "0" * len(trimmed_id):
        log_event(
            logger,
            logging.DEBUG,
            ObservabilityEvent(
