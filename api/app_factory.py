...
def _log_degraded_startup(phase: str, message: str, exc: BaseException) -> None:
    """Emit a bounded degraded-startup event for hosted fallback paths."""
    log_event(
        logger,
        logging.WARNING,
        ObservabilityEvent(
...
async def _verify_auth_database_for_startup() -> None:
    """Fail closed if credential schema verification stalls or detects drift."""
    try:
...
async def _perform_degradable_startup_reconciliation(
    settings: GraphLifecycleSettings,
    hosted_startup_degradation_allowed: bool,
) -> None:
    """Run startup reconciliation, degrading only for transient hosted failures."""
    try:
...
def _bootstrap_graph_state(hosted_startup_degradation_allowed: bool) -> None:
    """Load graph state, degrading only for hosted storage/bootstrap failures."""
    try:
        get_graph()
...
async def _initialize_application_state(
    ...
    await _verify_auth_database_for_startup()
    if has_persistence:
        await _perform_degradable_startup_reconciliation(settings, hosted_startup_degradation_allowed)
